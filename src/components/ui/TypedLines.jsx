  ...
  return (
    <div ref={ref}>
      {lines.map((tokens, lineIdx) => {
        ...
        const charsToShow = Math.max(0, Math.min(visibleChars - lineStart, lineLen))
        const isTypingThisLine = visibleChars > lineStart && visibleChars < lineStart + lineLen
        ...
        if (charsToShow >= lineLen && lineLen > 0) {
          ...
          content = tokens.map((tok, j) => {
            ...
            return cls ? <span key={j} className={cls}>{tok.text}</span> : tok.text
          })
        ...
        return (
          <div key={lineIdx} className="code-line">
            ...
            <span className="line-content">
              {content}
              {showCursor && (
                <span className={isDone ? 'typed-cursor typed-cursor--blink' : 'typed-cursor'} />
              )}
            </span>
          </div>
        )

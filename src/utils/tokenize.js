...
const CS_KEYWORDS = [
  'abstract','async','await','base','bool','break','byte','case','catch',
...
function buildShRules() {
  return [
    ...
    // shell keywords
    { re: /\b(if|then|else|elif|fi|for|while|do|done|case|esac|in|function|return|local|export|echo|read|set|unset|source|exit|shift|break|continue|true|false)\b/g, type: 'keyword' },
    // shell builtins / common commands
    { re: /\b(cat|ls|cd|mkdir|rm|cp|mv|grep|sed|awk|curl|wget|git|npm|node|bash|sh|printf|test)\b/g, type: 'function' },
    // operators
    { re: /[|&;><()\[\]=!+\-*\/{}]/g,    type: 'punctuation' },
  ]
}
...
function getRules(lang) {
  if (!RULE_CACHE[lang]) {
    switch (lang) {
      case 'ts':   RULE_CACHE[lang] = buildTsRules();   break
  ...
  rules.forEach(r => { r.re.lastIndex = 0 })
  return rules
}
